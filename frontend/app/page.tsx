...
export default function App() {
  ...
  // 🔹 Purchase ticket
  async function purchaseTicket(eventId: number, price: string) {
    if (!window.ethereum) {
      ...
      <div className="flex flex-col min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-100">
        ...
        <main className="flex-grow container mx-auto px-4 py-8">
          ...
          {address && !hasEnoughEth() && showEthWarning && (
            <div className="mb-4 p-4 bg-yellow-100 border border-yellow-400 text-yellow-700 rounded relative">
              <button
                ...
                <form onSubmit={createEvent} className="space-y-6">
                  ...
                  <button
                    ...
                    className={`w-full px-4 py-2 bg-blue-600 text-white rounded-lg ${
                      minting ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-700'
                    }`}
                  >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {events.map((event) => (
                    <div key={event.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                      ...
                      <p className="text-gray-600 dark:text-gray-400 mb-2">Date: {event.date}</p>
                      <p className="text-gray-600 dark:text-gray-400 mb-2">Location: {event.location}</p>
                      <p className="text-blue-600 font-bold mb-2">{event.price} ETH</p>
                      ...
                      >
                        {minting ? 'Processing...' : event.soldTickets >= event.totalTickets ? 'Sold Out' : 'Purchase Ticket'}
                      </button>
                    </div>
                  ))}
                </div>
